...
#------------------------------------------------------------------------------------------
# The One inflated BurrXII distribution (OIUBXII) - basic functions
#------------------------------------------------------------------------------------------
...
OIUBXII<-function (mu.link = "logit", sigma.link = "log", nu.link = "logit") 
{
  ...
  structure(list(family = c("OIUBXII", "One inflated BurrXII"), 
                 ...
                 dldm = function(y, mu, sigma) {
                   tx<-function(x){1+log(1/x)^sigma}
                   tau=.5   
                   dldm <- ifelse((y == 0 | y == 1), 0, eval((sigma * log(1 / mu)^(-1 + sigma)) / (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)) + 
                   ...
                   tx<-function(x){1+log(1/x)^sigma}
                   tau=.5   
                   dldm <- eval((sigma * log(1 / mu)^(-1 + sigma)) / (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)) + 
                                  (sigma * log(1 / mu)^(-1 + sigma) * log(tau) * log(1 + log(1 / y)^sigma)) / 
                                  (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)^2))  
                   ...
                   tx<-function(x){1+log(1/x)^sigma}
                   tau=.5
                   dldd <- ifelse((y == 0 | y == 1), 0, eval(1/sigma+log(log(1/y))-log(log(mu^-1))*(tx(mu)-1)/(tx(mu)*log(tx(mu)))-
                                                               (tx(y)-1)*log(log(1/y))/tx(y)-
                                                               log(1/tau)/log(tx(mu))*(tx(y)^-1)*(tx(y)-1)*log(log(1/y))+ 
                                                               log(1/tau)*(tx(mu)-1)*log(log(1/mu))/(tx(mu)*log(tx(mu))^2)*log(tx(y))))
                   dldd
                 }, d2ldd2 = function(y, mu, sigma) { 
                   tx<-function(x){1+log(1/x)^sigma}
                   tau=.5                  
                   dldd <- ifelse(is.nan(eval(1/sigma+log(log(1/y))-log(log(mu^-1))*(tx(mu)-1)/(tx(mu)*log(tx(mu)))-
                                                (tx(y)-1)*log(log(1/y))/tx(y)-
                                                log(1/tau)/log(tx(mu))*(tx(y)^-1)*(tx(y)-1)*log(log(1/y))+ 
                                                log(1/tau)*(tx(mu)-1)*log(log(1/mu))/(tx(mu)*log(tx(mu))^2)*log(tx(y)))), 0, eval(1/sigma+log(log(1/y))-log(log(mu^-1))*(tx(mu)-1)/(tx(mu)*log(tx(mu)))-
                                                                                                                                    (tx(y)-1)*log(log(1/y))/tx(y)-
                                                                                                                                    log(1/tau)/log(tx(mu))*(tx(y)^-1)*(tx(y)-1)*log(log(1/y))+ 
                                                                                                                                    log(1/tau)*(tx(mu)-1)*log(log(1/mu))/(tx(mu)*log(tx(mu))^2)*log(tx(y))))  
...
for (i in 1:100) {
  ...
  fit1<-gamlss(y~1,family="OIUBXII",trace=F)
