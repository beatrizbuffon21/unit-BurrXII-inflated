...
dZIUBXII<-function (x, mu = 0.7, sigma = 2.1, nu = 0.1, log = FALSE) 
{
  ...
  log.kum =log(log(tau^(-sigma))/(log(1 + log(mu^(-1))^sigma))) - log(x) +
    ...
    (sigma-1)*(log(-log(x)))
  log.lik <- ifelse(x == 0, log(nu), log(1 - nu) + log.kum)
  if (log == FALSE) 
    fy <- exp(log.lik)
  else fy <- log.lik
...
ZIUBXII<-function (mu.link = "logit", sigma.link = "log", nu.link = "logit") 
{
  ...
  dstats <- checklink("sigma.link", "ZIUBXII", substitute(sigma.link), 
                      c("inverse", "log", "identity"))
  vstats <- checklink("nu.link", "ZIUBXII", substitute(nu.link), 
                      c("logit", "probit", "cloglog", "log", "own"))
  structure(list(family = c("ZIUBXII", "Zero inflated BurrXII"), 
                 ...
                 dldm = function(y, mu, sigma) {
                   tau=.5   
                   dldm <- ifelse((y == 0), 0, (sigma * log(1 / mu)^(-1 + sigma)) / (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)) + 
                                    (sigma * log(1 / mu)^(-1 + sigma) * log(tau) * log(1 + log(1 / y)^sigma)) / 
                                    (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)^2))
                   dldm
                 }, d2ldm2 = function(y, mu, sigma) { 
                   tau=.5   
                   dldm <- (sigma * log(1 / mu)^(-1 + sigma)) / (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)) + 
                     (sigma * log(1 / mu)^(-1 + sigma) * log(tau) * log(1 + log(1 / y)^sigma)) / 
                     (mu * (1 + log(1 / mu)^sigma) * log(1 + log(1 / mu)^sigma)^2)  
                   ...
                   tau=.5
                   dldd <- ifelse((y == 0), 0, 1/sigma+log(log(1/y))-log(log(mu^-1))*((1+log(1/mu)^sigma)-1)/((1+log(1/mu)^sigma)*log((1+log(1/mu)^sigma)))-
                                    ((1+log(1/y)^sigma)-1)*log(log(1/y))/(1+log(1/y)^sigma)-
                                    log(1/tau)/log((1+log(1/mu)^sigma))*((1+log(1/y)^sigma)^-1)*((1+log(1/y)^sigma)-1)*log(log(1/y))+ ## aqui
                                    log(1/tau)*((1+log(1/mu)^sigma)-1)*log(log(1/mu))/((1+log(1/mu)^sigma)*log((1+log(1/mu)^sigma))^2)*log((1+log(1/y)^sigma))
                   )
                   dldd
                 }, d2ldd2 = function(y, mu, sigma) { 
                   tau=.5                  
                   dldd <- ifelse(is.nan(1/sigma+log(log(1/y))-log(log(mu^-1))*((1+log(1/mu)^sigma)-1)/((1+log(1/mu)^sigma)*log((1+log(1/mu)^sigma)))-
                                           ((1+log(1/y)^sigma)-1)*log(log(1/y))/(1+log(1/y)^sigma)-
                                           log(1/tau)/log((1+log(1/mu)^sigma))*((1+log(1/y)^sigma)^-1)*((1+log(1/y)^sigma)-1)*log(log(1/y))+ ## aqui
                                           log(1/tau)*((1+log(1/mu)^sigma)-1)*log(log(1/mu))/((1+log(1/mu)^sigma)*log((1+log(1/mu)^sigma))^2)*log((1+log(1/y)^sigma))
                   ), 0, 1/sigma+log(log(1/y))-log(log(mu^-1))*((1+log(1/mu)^sigma)-1)/((1+log(1/mu)^sigma)*log((1+log(1/mu)^sigma)))-
                     ((1+log(1/y)^sigma)-1)*log(log(1/y))/(1+log(1/y)^sigma)-
                     log(1/tau)/log((1+log(1/mu)^sigma))*((1+log(1/y)^sigma)^-1)*((1+log(1/y)^sigma)-1)*log(log(1/y))+ ## aqui
                     log(1/tau)*((1+log(1/mu)^sigma)-1)*log(log(1/mu))/((1+log(1/mu)^sigma)*log((1+log(1/mu)^sigma))^2)*log((1+log(1/y)^sigma))
                   )  
                   ...
                   -2 * dZIUBXII(y, mu, sigma, nu, log = TRUE)
                 }, rqres = expression({
                   uval <- ifelse(y == 0, nu * runif(length(y), 0, 1), 
                                  (1 - nu) * pZIUBXII(y, mu, sigma, nu))
                   rqres <- qnorm(uval)
                 }), 
...
for (i in 1:R) {
  ...
  fit1 <- try(gamlss(y ~ X, sigma.formula = ~ X, 
                     nu.formula = ~ X, 
                     family = ZIUBXII(), trace=F),silent=T)
